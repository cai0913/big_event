        ...
        return {
            ...
            pwdFormRules:{
                old_pwd:[
                    ...
                    {pattern:/^\S{6,15}$/,message:'密码长度必须是6~15位的非空字符串',triggle:'blur'}
                ],
                new_pwd:[
                    {required:true,message:'请输入新密码',triggle:'blur'},
                    {pattern:/^\S{6,15}$/,message:'密码长度必须是6~15位的非空字符串',triggle:'blur'},
                    {validator:samePwd,triggle:'blur'}
                ],
                re_pwd:[
                    {required:true,message:'请再次确认密码',triggle:'blur'},
                    {pattern:/^\S{6,15}$/,message:'密码长度必须是6~15位的非空字符串',triggle:'blur'},
                    {validator:rePwd,triggle:'blur'}
                ]
            }
        }
    },
      ...
      updatePwdFn(){
        this.$refs.pwdFormRef.validate(async valid => {
          if(valid){
            ...
            if(res.code !== 0) return this.$message.error('原密码输入错误！')
